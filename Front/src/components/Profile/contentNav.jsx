...
function ContentLogo() {
    ...
    const { createImages, getImages, updateImages } = useImages();
        ...
        <div className="flex-grow max-w-xl space-y-4 p-4 mx-auto">
            ...
            <Formik
                ...
                onSubmit={async (values, actions) => {
                    console.log(values);
                    if (params.id) {
                        await updateImages(params.id, values)
                        navigate("/profile")
                    } else {
                        await createImages(values);
                    }
                    actions.resetForm();
            ...
            >
                {({ handleChange, handleSubmit, values, isSubmitting, setFieldValue }) => (
                    <Form className="w-max-w space-y-2 mb-10" onSubmit={handleSubmit}>
                        <label className="flex flex-col">Logo barra de navegación</label>
