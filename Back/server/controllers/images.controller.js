...
import { uploadImageNav } from '../libs/cloudinarymultimedia.js'
...
export const getImages = async (req, res) => {
    try {
        const [result] = await pool.query(
            "SELECT * FROM images WHERE section = 'logo_navbar' ORDER BY id DESC LIMIT 1;"
        );
        res.json(result);
    } catch (error) {
...
export const createImages = async (req, res) => {
    try {
        ...
        if (req.files && req.files.image) {
            const result = await uploadImageNav(req.files.image.tempFilePath);
